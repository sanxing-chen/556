...
typedef struct G {
    ...
    void dfscentroidsz(int at, int par) {
        csz[at] = 1;
        for (int x = ghead[at]; x != -1; x = gnxt[x]) {
            int to = gto[x];
            if (wascentroid[to] || to == par) continue;
            dfscentroidsz(to, at);
            csz[at] += csz[to];
        }
    }
    int dfscentroid(int at, int par, int cn) {
        for (int x = ghead[at]; x != -1; x = gnxt[x]) {
            int to = gto[x];
            if (wascentroid[to] || to == par) continue;
            if (2 * csz[to] > cn) return dfscentroid(to, at, cn);
        }
        return at;
    }
} G;
...
typedef struct T {
    int n;
    int root, chead[MAXTN], cnxt[MAXTN], par[MAXTN];
    char cpar[MAXTN];
    void init(const G &g, int rt) {
        n = g.n, root = rt;
        REP(i, n) chead[i] = -1;
        par[rt] = -1, cpar[rt] = '-';
        dfsinit(g, rt);
    }
    void dfsinit(const G &g, int at) {
        for (int x = g.ghead[at]; x != -1; x = g.gnxt[x]) {
            int to = g.gto[x];
            if (to == par[at]) continue;
            par[to] = at, cpar[to] = g.gc[x >> 1];
            dfsinit(g, to);
            cnxt[to] = chead[at];
            chead[at] = to;
        }
    ...
    typedef struct LadderSeg {
        int id, s, t;
        LadderSeg(int id, int s, int t)
            : id(id)
            , s(s)
            , t(t) {}
    } LadderSeg;
    ...
    void calcladders() {
        d[root] = 0;
        dfslongpaths(root);
        nladders = ladderoff[0] = 0;
        REP(i, n) if (par[i] == -1 || longpath[par[i]] != i) {
            int id = nladders++;
            ladderoff[id + 1] = ladderoff[id];
    ...
    void dfslongpaths(int at) {
        h[at] = 1, longpath[at] = -1;
        for (int to = chead[at]; to != -1; to = cnxt[to]) {
            d[to] = d[at] + 1;
            dfslongpaths(to);
            if (h[to] + 1 > h[at]) h[at] = h[to] + 1, longpath[at] = to;
        }
    }
    int lca(int u, int v) {
        while (true) {
            ...
            int k = 0;
            while (dmn + (1 << k) < d[u] && dmn + (1 << k) < d[v]) ++k;
            for (--k; k >= 0; --k)
                if (dmn + (1 << k) < d[u] && dmn + (1 << k) < d[v] && laddernodes[uoff + dmn + (1 << k) - dutop] == laddernodes[voff + dmn + (1 << k) - dvtop])
                    dmn += 1 << k;
            if (laddernodes[uoff + dmn + 1 - dutop] == laddernodes[voff + dmn + 1 - dvtop])
                return laddernodes[uoff + dmn + 1 - dutop];
            else
                return laddernodes[uoff + dmn - dutop];
        }
    }
    vector<LadderSeg> ladderpath(int u, int v) {
        ...
        while (u != w) {
            ...
            if (dtop >= d[w]) {
                ret.PB(LadderSeg(id, d[u] - dtop, 0));
                u = laddernodes[off];
                continue;
            }
            ret.PB(LadderSeg(id, d[u] - dtop, d[w] - dtop));
            u = laddernodes[off + d[w] - dtop];
        }
        int revoff = SZ(ret);
        while (v != w) {
            int id = ladderid[v], off = ladderoff[id], dtop = d[laddernodes[off]];
            if (dtop >= d[w]) {
                ret.PB(LadderSeg(id, 0, d[v] - dtop));
                v = laddernodes[off];
                continue;
            }
            ret.PB(LadderSeg(id, d[w] - dtop, d[v] - dtop));
            v = laddernodes[off + d[w] - dtop];
        }
...
typedef struct SuffTree {
        ...
        for (int i = 0; i < slen; ++i) {
            ...
            while (rem > 0) {
                ...
                if (tonode == -1) { // atlen should be 0
                    ...
                    if (prev != -1) nodes[prev].suffix = atnode;
                    prev = atnode;
                    ...
                    if (prev != -1) nodes[prev].suffix = branch;
                    prev = branch;
                }
                --rem;
                if (atnode == root) {
    ...
    int sa[MAXSTLEN], lcp[MAXSTLEN];
    int nsa;
    int rmqjump[MAXSTLEN][MAXLGSTLEN + 1];
    int lg[MAXSTLEN + 1];
    void createsa() {
        nsa = 0;
        dfscreatesa(root, 0, -1);
        FOR(i, 1, nsa) rmqjump[i][0] = lcp[i];
        for (int k = 1; (1 << k) <= nsa; ++k) REPE(i, nsa - (1 << k))
        rmqjump[i][k] = min(rmqjump[i][k - 1], rmqjump[i + (1 << (k - 1))][k - 1]);
        lg[1] = 0;
        FORE(i, 2, nsa) {
            lg[i] = lg[i - 1];
            while ((2 << lg[i]) <= i) ++lg[i];
    ...
    void dfscreatesa(int node, int depth, int clcp) {
        if (nodes[node].end == slen) {
            int idx = nodes[node].start - depth;
            lcp[nsa] = clcp;
            sa[idx] = nsa++;
        }
        for (int i = 0; i < 27; ++i)
            if (nodes[node].childs[i] != -1) {
                dfscreatesa(nodes[node].childs[i], depth + nodes[node].end - nodes[node].start, clcp);
                clcp = depth + nodes[node].end - nodes[node].start;
            }
    }
    int rmq(int a, int b) {
        int k = lg[b + 1 - a];
        return min(rmqjump[a][k], rmqjump[b + 1 - (1 << k)][k]);
    }
    void dfsprint(int node, int depth) {
        for (int i = 0; i < depth; ++i) printf("  ");
        printf("'");
        for (int i = nodes[node].start; i < nodes[node].end; ++i) printf("%c", s[i]);
        printf("'");
        if (nodes[node].end != slen) printf(" -> %d", node);
        if (nodes[node].suffix != -1) printf(" [%d]", nodes[node].suffix);
        puts("");
        for (int i = 0; i < 27; ++i)
            if (nodes[node].childs[i] != -1) dfsprint(nodes[node].childs[i], depth + 1);
    }
    void print() {
        printf("%s\n", s);
        dfsprint(root, 0);
        REP(i, slen) printf("%d ", sa[i]);
        puts("");
        REP(i, slen) printf("%d ", lcp[i]);
        puts("");
    }
} SuffTree;
SuffTree st;
...
void createstr() {
    ...
    REP(id, t.nladders) {
        ...
        dnoff[id] = st.slen;
        REP(j, len - 1)
        st.s[st.slen++] = t.cpar[t.laddernodes[t.ladderoff[id] + j + 1]];
        upoff[id] = st.slen;
        REP(j, len - 1)
        st.s[st.slen++] = t.cpar[t.laddernodes[t.ladderoff[id] + len - j - 1]];
    }
    st.s[st.slen++] = '{';
    st.s[st.slen] = '\0';
    st.build();
...
void printpath(const vector<pair<int, int>> &path) {
    REPSZ(i, path) {
        if (i != 0) printf(" ");
        FOR(j, path[i].first, path[i].second) printf("%c", st.s[j]);
    }
}
vector<pair<int, int>> pathstrs(int u, int v) {
    vector<T::LadderSeg> res = t.ladderpath(u, v);
    ...
    REPSZ(i, res) {
        ...
        if (res[i].s > res[i].t)
            ret.PB(MP(upoff[res[i].id] + len - res[i].s - 1, upoff[res[i].id] + len - res[i].t - 1));
    }
    return ret;
}
...
int _pathcmp(const vector<pair<int, int>> &a, const vector<pair<int, int>> &b) {
    ...
    while (ai < SZ(a) && bi < SZ(b)) {
        ...
        int cmp = scmp(a[ai].first + aoff, b[bi].first + boff, len);
        if (cmp != 0) return cmp;
        aoff += len;
        if (a[ai].first + aoff >= a[ai].second) ++ai, aoff = 0;
        boff += len;
        if (b[bi].first + boff >= b[bi].second) ++bi, boff = 0;
    }
    bool adone = ai >= SZ(a), bdone = bi >= SZ(b);
    if (adone != bdone) return adone ? -1 : +1;
    return 0;
}
int pathcmp(const vector<pair<int, int>> &a, const vector<pair<int, int>> &b) {
    int ret = _pathcmp(a, b);
    return ret;
}
...
vector<pair<int, int>> triepaths[MAXN];
int ntriepaths;
int triepathidx[MAXN];
int createtrienode() {
    int ret = ntrie++;
    REP(i, 26) trienxt[ret][i] = -1;
    trienodehead[ret] = -1;
    return ret;
}
void dfscreatetrie(int at, int par, int z) {
    ...
    for (int x = g.ghead[at]; x != -1; x = g.gnxt[x]) {
        int to = g.gto[x];
        if (to == par || g.wascentroid[to]) continue;
        char c = g.gc[x >> 1];
        if (trienxt[z][c - 'a'] == -1) trienxt[z][c - 'a'] = createtrienode();
        dfscreatetrie(to, at, trienxt[z][c - 'a']);
    }
}
void dfscreatetriepaths(int z, int c) {
    for (int x = trienodehead[z]; x != -1; x = trienodenxt[x])
        triepathidx[x] = ntriepaths++, triepaths[triepathidx[x]] = pathstrs(c, x);
    REP(i, 26) if (trienxt[z][i] != -1) dfscreatetriepaths(trienxt[z][i], c);
}
int bit[MAXN + 1];
void bitmod(int idx, int by) {
    ++idx;
    while (idx <= ntriepaths) {
        bit[idx] += by;
        idx += idx & -idx;
    }
}
...
void dfsbitmod(int at, int par, int by) {
    bitmod(triepathidx[at], by);
    for (int x = g.ghead[at]; x != -1; x = g.gnxt[x]) {
        int to = g.gto[x];
        if (to == par || g.wascentroid[to]) continue;
        dfsbitmod(to, at, by);
...
int calcpathsthroughcentroid(int c, int u, int v) {
    ...
    int uplen = 0;
    REPSZ(i, uppath) uplen += uppath[i].second - uppath[i].first;
    vector<pair<int, int>> qpath1, qpath2;
    int rem = uplen;
    REPSZ(i, qpath) {
        int curlen = qpath[i].second - qpath[i].first;
        if (rem >= curlen) {
            qpath1.PB(qpath[i]);
            rem -= curlen;
        } else if (rem > 0) {
            qpath1.PB(MP(qpath[i].first, qpath[i].first + rem));
            qpath2.PB(MP(qpath[i].first + rem, qpath[i].second));
            rem = 0;
        } else
            qpath2.PB(qpath[i]);
    }
    ...
    if (cmp1 > 0) return bitget(ntriepaths - 1);
    if (cmp1 < 0) return 0;
    int l = -1, h = ntriepaths;
    while (l + 1 < h) {
        int m = l + (h - l) / 2;
        if (pathcmp(triepaths[m], qpath2) < 0)
            l = m;
        else
            h = m;
    }
...
void dfscalcpathsthroughcentroid(int at, int par, int c) {
    REPSZ(i, q[at]) {
        int u = at, v = q[at][i].first, idx = q[at][i].second;
        ans[idx] += calcpathsthroughcentroid(c, u, v);
    }
    for (int x = g.ghead[at]; x != -1; x = g.gnxt[x]) {
        int to = g.gto[x];
        if (to == par || g.wascentroid[to]) continue;
        dfscalcpathsthroughcentroid(to, at, c);
    }
}
void calccentroid(int c) {
    ntrie = 0;
    int trieroot = createtrienode();
    dfscreatetrie(c, -1, trieroot);
    ntriepaths = 0;
    dfscreatetriepaths(trieroot, c);
    memset(bit, 0, sizeof(bit));
    REP(i, ntriepaths) bitmod(i, +1);
    REPSZ(i, q[c]) {
        int u = c, v = q[c][i].first, idx = q[c][i].second;
        ans[idx] += calcpathsthroughcentroid(c, u, v);
    }
    for (int x = g.ghead[c]; x != -1; x = g.gnxt[x]) {
        int to = g.gto[x];
        if (g.wascentroid[to]) continue;
        dfsbitmod(to, c, -1);
